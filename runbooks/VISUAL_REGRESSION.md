...
| `work-goatbridge` | `/work/goatbridge/` | Goat Bridge heading, synthetic-input and hardware-validation boundary, and initial simulator media are visible |
...
- Keep Windows and genuine Linux baselines paired for every route and viewport once Linux evidence is generated. A Windows-only new-route baseline must remain explicitly documented as such and must never be presented as Linux evidence.
