...
As of 2026-05-26: B-014 (CLI Fleet Synchronization and MCP Rollout), B-015 (Remote Workstation Recovery and Operational Debugging), and B-020 (Cryo Flow Sim Stage 1) are approved. B-016 (HumanKaylee Portfolio Build) and B-017 (Creative Web Systems Atlas Demo) remain `reviewed` pending production evidence. Minimum launch gate requires 4 approved `publish` candidates; current count is 3 approved plus 1 reviewed.
