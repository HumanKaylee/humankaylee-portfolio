...
| Rust API deployment                  | B-058 / #64     | Blocked until API host decision, provider project, public API origin, secret store, and contact handling decision exist.                        | Keep Shuttle as legacy-only, compare Fly.io, Railway, or another approved host, record `GET /api/health`, `HK_API_ALLOWED_ORIGINS`, `HK_API_CONTACT_DELIVERY_MODE`, CORS checks, and rollback target fields.             | Public or approved-preview `GET /api/health`, CORS smoke-check output, secret storage record, deployment ID, API origin, and rollback target.                     |
