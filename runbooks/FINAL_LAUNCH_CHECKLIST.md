...
| At least four case studies are live and redacted      | Blocked / not run         | Current approved launch case studies: 0 in `runbooks/CONTENT_REDACTION_STATUS.md`; Privacy Redaction Rule cells must be populated, and `reviewed` case studies are not launch-eligible.     | Complete redaction approval packets and inspect linked artifacts before changing any case study to `approved`.                                                     |
| Resume route and PDF production smoke evidence exists | Blocked / not run         | Local resume source is approved and the PDF asset checksum is recorded in `runbooks/LAUNCH_EVIDENCE.md`; no production frontend target exists.                                              | Smoke-test production `/resume/` and PDF link after frontend deploy.                                                                                               |
...
| Contact path works                                    | Blocked / not run         | Local contact fallback and API-down evidence exists; production contact handling is not approved. `pnpm phase7:contact-decision -- --mode defer --dry-run` can shape the required decision fields only. | Choose mailto-only launch exception or configure approved persistent contact store/provider, then record owner approval and production or owner-approved production-equivalent provider preview smoke evidence. |
...
| Static quality matrix gate is green                   | Pass for PR/local CI only | Phase 0 CI includes `Run static quality matrix gate`; the focused local command is `pnpm test:e2e -- --grep "@quality"`. This is local/PR CI evidence only, not production launch evidence. | Re-run after route, static-shell, privacy, route-coverage, accessibility, no-JS, or reduced-motion changes; production launch evidence remains separately blocked. |
