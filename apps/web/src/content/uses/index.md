...
sections:
  - label: "Hardware"
    items:
      ...
      - name: "Secondary headless server (ares-tron, Linux, Threadripper)"
        why: "Heavy compute, long-running collectors, and GPU workloads live here. LAN + Tailscale mesh keeps it reachable without VPN config overhead."
      - name: "Tailscale mesh"
        why: "Zero-config WireGuard overlay across all machines. MagicDNS means I SSH to hostnames rather than IPs even across networks."
  ...
  - label: "Languages and runtimes"
    items:
      ...
      - name: "Node / TypeScript"
        why: "Frontend build tooling and Astro. TypeScript with strict mode is a prerequisite — untyped JS has too high a per-change reasoning cost."
      - name: "Bash"
  ...
  - label: "Infrastructure"
    items:
      - name: "Cloudflare Pages + Workers"
        why: "Static frontend hosting with global CDN, zero cold-start, and free analytics. The build-and-deploy pipeline fits in a single pnpm command."
      - name: "Hetzner CX22"
        why: "Flat-rate Linux VPS for the Axum API. $3.59/month, no cold-start, predictable latency. Caddy handles TLS termination."
      - name: "GitHub Actions"
      ...
      - name: "SQLite + Litestream"
        why: "Contact store and local data pipelines. Litestream replicates WAL frames to R2 without a managed database subscription."
  - label: "CLI tooling"
    items:
      - name: "ripgrep (rg)"
